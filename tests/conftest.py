...
# ---------------------------------------------------------------------------
# FakeLLM -- a scripted stand-in for `groq.AsyncGroq`
# ---------------------------------------------------------------------------
...
class _FakeCompletions:
    ...
    async def create(self, *, model=None, messages=None, **kwargs):
        self._llm.calls.append(SimpleNamespace(model=model, messages=messages))

        if not self._llm.replies:
            raise AssertionError(
                f"FakeLLM: the orchestrator made an unexpected extra LLM call "
                f"(call #{len(self._llm.calls)}), but the script is exhausted.\n"
                f"Either the code under test changed how many times it calls the "
                f"model, or the test needs another scripted reply."
            )

        content = self._llm.replies.pop(0)
...
class FakeLLM:
    """Mimics the small slice of the Groq client that the app actually uses:
    ...
    exact branch. Running out of replies is a hard error, not a silent None --
...
def make_tutor(monkeypatch, fake_llm, notion):
    ...
    def _build(**initial_context):
        ...
        # The Groq client, bound into tutor_service's namespace at import time.
        monkeypatch.setattr(tutor, "client", fake_llm)
